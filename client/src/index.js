...
class App extends Component {
  ...
  componentDidMount() {
    ...
    if (history !== null && history !== 'empty') {
      ...
      axios.get(`/api/search/${JSON.parse(history)}`).then(res => {
  ...
  handleSubmit(e) {
    e.preventDefault();
    if (
      this.state.term !== this.state.searchHistory &&
      this.state.term !== ''
    ) {
      axios.get(`/api/search/${this.state.term}`).then(res => {
        console.log(res);
        this.setState({ data: res.data });
      });
      localStorage.setItem('prevSearch', 'empty');
    ...
    const render = data.map(x => {
      const result = (
        <Results
          ...
          rating={x.rating}
          phone={x.phone}
          location={x.location.display_address}
          rsvp={x.rsvp}
        />
      );
      return result;
